...
Capturing groups are used when we want the regex engine to return sub patterns in addition to the overall matching pattern. In regex pattern, a capturing group lives in a pair of round parentheses `(pattern)`.

The following example has two capturing groups and three strings are returned, The first string is `123-4567890`, which is the overall matching pattern and is called group 0. The second returned string is `123`, which matches capturing group 1 `(\\d{3})`. The third is `4567890`, which matches group 2 `(\\d{7})`.
...
We can give each or some capturing groups a name instead of just group 2 or 3.
...
The groups 1 and 2 can be referenced as `\\1` and `\\2` in back references. In the example below we switch the order of first name and last name using back references in a search and replacement.
